...
prepare_project_nginx_reverse_proxy_configs() {
  ...
  _nginx_proxy_config_filepath="${project_up_dir}/nginx-reversproxy/conf.d/${WEBSITE_HOST_NAME}.conf"
  # prepare http.conf.pattern or https.conf.pattern within docker-up directory
  copy_path_with_project_fallback "configs/infrastructure/nginx-reversproxy/${CONFIGS_PROVIDER_NGINX_PROXY}/conf.d/${WEBSITE_PROTOCOL}.conf.pattern" "${_nginx_proxy_config_filepath}"

  local _proxy_pass_host="${CONTAINER_WEB_NAME}"
  if [[ "${VARNISH_ENABLE}" == "yes" ]]; then
    _proxy_pass_host="${CONTAINER_VARNISH_NAME}"
  else
    _proxy_pass_host="${CONTAINER_WEB_NAME}"
  fi
  replace_value_in_file "${project_up_dir}/nginx-reversproxy/conf.d/${WEBSITE_HOST_NAME}.conf" "{{web_container_name}}" "${_proxy_pass_host}"
  ...
  replace_value_in_file "${_nginx_proxy_config_filepath}" "{{WEBSITE_EXTRA_HOST_NAMES_SS}}" "${_website_nginx_extra_host_names}"
