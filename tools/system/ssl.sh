...
#  sudo su -c "openssl req -x509 -nodes \
#            -newkey ec:<(openssl ecparam -name secp384r1) \
#            -keyout ${_target_key_path} \
#            -out ${_target_crt_path} \
#            -days 365 \
...
add_system_ssl_certificate() {
  ...
  if [[ ! -f "${_cert_source_path}" ]]; then
    show_error_message "Unable to remove CA certificate. Cert file does not exist at path \"${_cert_source_path}\"."
    exit 1
  fi
